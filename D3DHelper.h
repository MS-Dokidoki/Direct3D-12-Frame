    ...
    namespace Render
    {
        ...
        */
        ...
        struct FrameResource
        {
            ...
            /// @param pDevice D3D12 设备指针
            /// @param nByteSize 顶点数据块大小
            /// @param nCount 顶点数据块数量
            void InitDynamicVertexBuffer(ID3D12Device* pDevice, UINT nByteSize, UINT nCount)
            {
		...
		namespace Light
		{
			struct Material
			{
				std::string Name;					// 材质标识符
				
				int nNumFramesDirty;				
				int nMatCBIndex;					// 材质的常量缓冲区索引
				...
				float fRoughness;
				DirectX::XMFLOAT4X4 matMaterialTransform; 
			};
